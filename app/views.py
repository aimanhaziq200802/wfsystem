...
def all_quotations(request):
    ...
    context = {"user_quotations": user_quotations}
...
def create_request(request):
    ...
    if request.method == "POST":
        ...
        if not all(
            [title, remarks, verifier_username, entity_id, department_id, branch_id]
        ):
        ...
        try:
            ...
            for file in uploaded_files:
                attachment_instance = QuotationAttachment(
                    quotation=new_quotation, file=file
                )
...
"""
            send_notification_email(
                new_quotation.initiator.email, email_subject, email_body
            )
...
"""
            send_notification_email(
                new_quotation.verifier.email, email_subject, email_body
            )
...
def verification_queue(request):
    quotationList = Quotation.objects.filter(
        verifier=request.user, verify_status="PENDING"
    )
    context = {"quotationList": quotationList}
...
def verification_history(request):
    ...
    user_quotations = Quotation.objects.filter(
        verifier=request.user, verify_status__in=["VERIFIED", "REJECTED"]
    )

    # user_quotations = Quotation.objects.all()

    context = {"quotations": user_quotations}
...
def quotation_verify(request, qid):
    ...
    if request.method == "POST" and is_verifier:
        ...
        if (
            approver_username == "null" or not approver_username
        ):  # "null" is a placeholder for 'None' in your form
        ...
        if new_verify_status:
            ...
            q_detail.verify_remarks = request.POST.get("verify_remarks", "").strip()[
                :50
            ]
...
"""
                send_notification_email(
                    q_detail.initiator.email, email_subject, email_body
                )
                ######################################################################################################################
                # Notify the verifier
...
"""
                send_notification_email(
                    q_detail.verifier.email, email_subject, email_body
                )
                ######################################################################################################################
                # Notify the approver
...
"""
                send_notification_email(
                    q_detail.approver.email, email_subject, email_body
                )
...
def approval_queue(request):
    quotationList = Quotation.objects.filter(
        approver=request.user, approve_status="PENDING"
    )
    context = {"quotationList": quotationList}
...
def approval_history(request):
    user_quotations = Quotation.objects.filter(
        approver=request.user, approve_status__in=["APPROVED", "REJECTED"]
    )
    ...
    if request.method == "POST" and is_approver:
        ...
        if new_approve_status == "APPROVED":
            ...
            if forward_decision == "final_approver":
                ...
                try:
                    selected_final_approver = User.objects.get(
                        username=selected_final_approver_username
                    )
...
"""
                    send_notification_email(
                        selected_final_approver.email, email_subject, email_body
                    )

                except User.DoesNotExist:
                    messages.error(
                        request,
                        f"Final Approver {selected_final_approver_username} does not exist.",
                    )

            elif forward_decision == "finance":
                selected_finance_username = request.POST.get("finance_user")
                try:
                    selected_finance_user = User.objects.get(
                        username=selected_finance_username
                    )
...
"""
                    send_notification_email(
                        selected_finance_user.email, email_subject, email_body
                    )

                except User.DoesNotExist:
                    messages.error(
                        request,
                        f"Finance User {selected_finance_username} does not exist.",
                    )

            q_detail.save()
            messages.success(
                request, "Approval status and remarks updated successfully!"
            )
            return redirect("quotation_details", qid=qid)
...
def final_approval_queue(request):
    quotationList = Quotation.objects.filter(
        final_approver=request.user, final_approval_status="PENDING"
    )
    context = {"quotationList": quotationList}
...
def final_approval_history(request):
    # Fetch quotations that the current user has either verified or rejected.
    user_quotations = Quotation.objects.filter(
        final_approver=request.user, final_approval_status__in=["APPROVED", "REJECTED"]
    )
...
def final_approve(request, qid):
    ...
    if q_detail.final_approval_status != "PENDING":
        messages.error(
            request, "This quotation has already been processed by the final approver."
        )
        return redirect("quotation_details", qid=qid)
    ...
    if request.method == "POST" and is_final_approver:
        ...
        final_approval_remarks = request.POST.get("final_approval_remarks", "").strip()[
            :100
        ]
...
"""
                send_notification_email(
                    q_detail.final_approver.email, email_subject, email_body
                )
...
"""
                ...
                messages.error(
                    request, f"Finance User {selected_finance_username} does not exist."
                )
                return redirect("quotation_details", qid=qid)
            ...
            messages.success(
                request,
                "Final approval status, remarks, and finance user assignment were updated successfully!",
            )
            return redirect("quotation_details", qid=qid)
...
"""
            ...
            q_detail.save()
            messages.success(
                request, "Final approval status and remarks updated successfully!"
            )
            return redirect("quotation_details", qid=qid)
...
def finance_queue(request):
    ...
    quotationList = Quotation.objects.filter(
        finance_user=request.user, finance_status="PENDING"
    )
    context = {"quotationList": quotationList}
...
def finance_history(request):
    ...
    user_quotations = Quotation.objects.filter(
        finance_user=request.user, finance_status__in=["APPROVED", "REJECTED"]
    )
    context = {"quotations": user_quotations}
...
def finance_approve(request, qid):
    ...
    if q_detail.finance_status != "PENDING" or q_detail.finance_user != request.user:
        messages.error(
            request,
            "This quotation is either not pending for finance processing or you're not authorized to process it.",
        )
        return redirect("quotation_details", qid=qid)
...
"""
            send_notification_email(
                q_detail.finance_user.email, email_subject, email_body
            )
...
def export_quotation_to_excel(request, qid):
    ...
    headers = {
        ...
        "Verified At": q_detail.verified_at.strftime("%d-%m-%Y %H:%M %p")
        if q_detail.verified_at
        else "N/A",
        "Approval At": q_detail.approval_at.strftime("%d-%m-%Y %H:%M %p")
        if q_detail.approval_at
        else "N/A",
        ...
        "Final Approver": q_detail.final_approver.username
        if q_detail.final_approver
        else "N/A",
        "Final Approval Status": q_detail.get_final_approval_status_display(),
        "Final Approval At": q_detail.final_approval_at.strftime("%d-%m-%Y %H:%M %p")
        if q_detail.final_approval_at
        else "N/A",
        "Final Approval Remarks": q_detail.final_approval_remarks,
        "Finance Status": q_detail.get_finance_status_display(),
        "Finance Approval At": q_detail.finance_reviewed_at.strftime(
            "%d-%m-%Y %H:%M %p"
        )
        if q_detail.finance_reviewed_at
        else "N/A",
